...
import com.dvidr.storm.bolt.{CountMinSketchBolt, MyReportBolt, SplitSentenceBolt}
import storm.kafka.{KafkaSpout, SpoutConfig, ZkHosts}

object RunningTopicCountTopology {
  val SENTENCE_SPOUT_ID = "kafka-sentence-spout"
  val SPLIT_BOLT_ID = "split-bolt"
  val COUNT_BOLT_ID = "countminsketch-count-bolt"
  ...
  def main(args: Array[String]) {
    ...
    builder.setBolt(COUNT_BOLT_ID, new CountMinSketchBolt()).fieldsGrouping(SPLIT_BOLT_ID, new Fields("word"))
